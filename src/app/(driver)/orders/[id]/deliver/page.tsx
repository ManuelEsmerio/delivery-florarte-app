...
import { ArrowLeft, CheckCircle2, ShieldCheck, MessageSquare, UserCheck } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-screen animate-in fade-in duration-300 bg-white">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8 flex-1 pb-32">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Observaciones rápidas</Label>
            <div className="grid grid-cols-1 gap-2">
              {quickResponses.map((resp) => (
                <button
                  key={resp}
                  type="button"
                  onClick={() => setObservations(resp)}
                  className={cn(
              ...
              <button
                ...
                className={cn(
                  "w-full p-4 rounded-2xl text-left text-xs font-bold transition-all border-2",
                  observations === '' || !quickResponses.includes(observations)
